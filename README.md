...
└── dist/
    └── deepseek-ai-dsh-client-frieren-zzj-0.1.0-rc.17.tgz   # 打包产物（安装版用）
```
...
- 直接用本仓库自带的 `dist/deepseek-ai-dsh-client-frieren-zzj-0.1.0-rc.17.tgz`；
...
pnpm dsh plugin --profile web add "file:D:/JavaCode/ds-h/dsh-frieren-zzj/dist/deepseek-ai-dsh-client-frieren-zzj-0.1.0-rc.17.tgz"
```
...
pnpm add "file:D:/JavaCode/ds-h/dsh-frieren-zzj/dist/deepseek-ai-dsh-client-frieren-zzj-0.1.0-rc.17.tgz"
```
...
1. **升版本号**：改 `frieren-zzj/package.json` 的 `version`（如 `0.1.0-rc.17` → `0.1.0-rc.18`）。**必须升**：pnpm 按 lockfile 里的 integrity 校验 tgz，同版本号的新 tgz 不会被重新安装；
2. 重新打包（见「从源码打包」），得到 `dist/deepseek-ai-dsh-client-frieren-zzj-0.1.0-rc.17.tgz`；
3. 重装：
