...
dsh-芙莉莲-zzj —— 芙莉莲 × 辛美尔（葬送のフリーレン）装饰主题：别名令牌覆盖层、全局样式表与装饰插槽条目。浏览器半部通过主题服务叠加令牌层、注入全局样式表（奇幻衬线标题、金紫渐变滚动条、星光、苍月草飘花、魔法阵），并向各插槽注册帧级装饰层、勇者金戒指印章、会话头部徽记与轮换的辛美尔铭文。初始状态为无壁纸，用户可自行上传图片作为整体背景并调节透明度。
...
- 自定义壁纸上传（压缩为 JPEG data URL 存入设置文档，再次上传即替换，「移除壁纸」一键清除）。初始状态为**无壁纸**，用户上传后壁纸作为整体背景以独立固定层渲染（`position:fixed; z-index:-1`），不干扰 `body` 默认布局；
- **壁纸透明度**（0%–100%）：滑块实时调节壁纸背景的 CSS `opacity`，0% 完全透明、100% 完全不透明。仅在设置了壁纸时显示；
...
- **恢复默认设置**（分区底部按钮）：整段重置为默认值（无壁纸、玻璃材质、装饰全开、随机台词、清除自定义壁纸与透明度恢复 100%）并重新开启插件。
